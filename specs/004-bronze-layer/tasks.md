...
- [ ] T001 Verify Databricks CLI/MCP authentication against the `~/.databrickscfg` `[DEFAULT]` profile still resolves to the workspace (reused from features 002-003) — prerequisite for every task below — **BLOCKED**: `databricks` CLI is not on PATH in this session (no MCP tool or `databricks-sdk`/`databricks-cli` Python package available either); see implementation notes below
...
- [ ] T004 [US1] Upload `rename_landing_schema.py` to the workspace and run it via `databricks jobs submit` on serverless compute (depends on T003) — **BLOCKED** by T001 (no Databricks access this session)
- [ ] T005 [US1] Review the run output and confirm all 5 files are listed at `ifood_case.landing.yellow_taxi_raw` with sizes matching feature 002's records, and the old `bronze`-named schema no longer exists (SC-001) (depends on T004) — **BLOCKED** by T004
...
- [X] T010 [US3] Extend `ingest_bronze.py`: write the deduplicated DataFrame to the managed Delta table `ifood_case.bronze.yellow_taxi_trips`, and compute/report `rows_read`, `rows_written`, `duplicates_removed`, `schema_validation_status` (FR-002/FR-007, data-model.md "Bronze Ingestion Run") (depends on T009)
- [ ] T011 [US3] Upload `ingest_bronze.py` to the workspace and run it via `databricks jobs submit` on serverless compute (depends on T010) — **BLOCKED** by T001 (no Databricks access this session)
- [ ] T012 [US3] Review the run output: confirm `duplicates_removed = 0` (matching feature 003's finding of 0 full-row duplicates in every month) and `rows_written` equals feature 003's total (16,186,386) minus `duplicates_removed` (SC-003) (depends on T011) — **BLOCKED** by T011
...
- [ ] T014 Write `specs/004-bronze-layer/ingestion-log.md` from T011's JSON output (`rows_read`, `rows_written`, `duplicates_removed`, `schema_validation_status`, `executed_at`) per research.md §5 (depends on T011) — **BLOCKED** by T011 (would require fabricating run numbers otherwise)
- [ ] T015 [P] Add an entry to `DECISOES_PROJETO.md` documenting the schema-rename workaround actually used (copy+verify+delete, since Unity Catalog has no `ALTER SCHEMA ... RENAME TO`) per Constitution Principle IV — same convention as feature 002's §2.1-2.3 entries (depends on T005) — **BLOCKED** by T005 (the mechanism is designed and coded, but "actually used" requires it to have actually run)
- [ ] T016 [P] Run `specs/004-bronze-layer/quickstart.md` end-to-end as final validation of all four user stories together (depends on T005, T012, T013) — **BLOCKED** by T005/T012/T013
