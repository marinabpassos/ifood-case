# Implementation Plan: POC App Chat — Consumo & Diferencial
...
both the consumption interface and the case's differentiator content at
...
SC-001. No concurrency/throughput target: this is a differentiator
prototype for a case evaluator to try, not a production service sized
...
pre-approved by the constitution's own framing of this feature as
differentiator content; each is itemized in Complexity Tracking below,
not silently introduced. Re-checked after Phase 1 below.
...
│       └── examples.md         # US2 / FR-004, FR-005, FR-006: curated example questions with their real generated SQL + executed answers, run against the actually-deployed app, clearly labeled as differentiator content
```
