...
| II. Data Contracts First | No | `_pipeline_run_log` is an operational/observability table, not a consumption-layer table governed by a Principle-II data contract the way `yellow_taxi_trips` is — it has no business schema to contract, only an operational log shape (fixed by this plan's data-model.md instead). |
